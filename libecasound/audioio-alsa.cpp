...
void ALSA_PCM_DEVICE::open(void) throw(ECA_ERROR*) {
  ...
  if (io_mode() == io_read) {
    ::snd_pcm_capture_time(audio_fd, 1);
    ...
    ::snd_pcm_playback_time(audio_fd, 1);
...
long int ALSA_PCM_DEVICE::read_samples(void* target_buffer, 
				 long int samples) {
  return(::snd_pcm_read(audio_fd, target_buffer, frame_size() * samples) / frame_size());
}

void ALSA_PCM_DEVICE::write_samples(void* target_buffer, long int samples) {
  ::snd_pcm_write(audio_fd, target_buffer, frame_size() * samples);
}
