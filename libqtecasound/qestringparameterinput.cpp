// ------------------------------------------------------------------------
// qestringparameterinput.cpp: Input widget for dynamic objects with string
//                             parameters
...
#include "qestringparameterinput.h"

QEStringParameterInput::QEStringParameterInput (DYNAMIC_OBJECT<string>* op, QWidget *parent, const char *name) 
  : QEInput(parent, name),
    object_rep(op) {
...
void QEStringParameterInput::init_layout(void) {
  ...
  if (object_rep != 0) {
    cop_desc->setText(object_rep->name().c_str());
...
void QEStringParameterInput::update_results(void) {
  if (object_rep == 0) return;
  
  for(int n = 0; n < object_rep->number_of_params(); n++) {
    object_rep->set_parameter(n + 1, string(inputlist[n]->text()));
  }
}
